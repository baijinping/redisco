...
class ModelSet(Set):
    def __init__(self, model_class):
        ...
        self._db = redisco.get_client(model_class._meta['db'])
        self._filters = {}
        self._exclusions = {}
        self._zfilters = []
        self._ordering = []
    ...
    def __iter__(self):
        ###########################
        # not raise Exception when self._set is None
        # bai jin ping, 2016/6/1
    ...
    def get_or_create(self, need_created=False, **kwargs):
        """
        Return an element of the collection or create it if necessary.

        :param need_created: 返回是否新建的状态
        ...
        if o:
            return o if not need_created else (o, False)
        else:
            o = self.create(**kwargs)
            return o if not need_created else (o, True)
    ...
    def _add_set_filter(self, s):
        ...
        return Set(new_set_key, db=self.db)
    ...
    def _set_with_ordering(self, skey):
        ...
        for ordering, alpha in self._ordering:
            ...
            self._expire_or_delete.append(new_set_key)
            return List(new_set_key, db=self.db)
